...
<?php
        ...
        foreach($post_categories as $c){
            ...
            switch($cat->slug){
                ...
                case 'sucursales-2':
                    ...
                    break;
                case 'esquelas':
                    $dev = sprintf('%esquelas', esc_url(home_url('/')));
                break;
                case 'esquelas-digitales':
                    $dev = sprintf('%esquelas-digitales', esc_url(home_url('/')));
                break;
                default:
    ...
    function translateSpanish($str){
        $engStr = array('January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December');
        $espStr = array('Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre');

        $translate = str_replace($engStr, $espStr, $str);

        return $translate;

    }

    add_filter('date', 'translateSpanish');
    add_filter('get_the_time', 'translateSpanish');
    add_filter('the_date', 'translateSpanish');
    add_filter('get_comment_date', 'translateSpanish');
    add_filter('get_comment_time', 'translateSpanish');
    ...
    function calendario($inicio, $fin){
        ...
        foreach ($dev as $d):
            ...
            ?>
            <a href="<?php hurl(); printf("%s/%s",$fecha->format("Y"), $fecha->format("m")); ?>" data-date="<?php printf("%s_%s", $fecha->format("Y"), $fecha->format("m")); ?>"><?php echo $meses[$fecha->format("n")-1]; ?></a>
            <?php
        endforeach;
        ?>
            </div>
    ...
    ?>
            <div class="archives">
                <div class="year">
                    <a class="move" data-year="<?php echo date("Y", strtotime($start . " -1 year")); ?>"><</a>
                    <a class="current"><?php echo $fecha->format("Y"); ?></a>
                    <a class="move" data-year="<?php echo date("Y", strtotime($end . " +1 year")); ?>">></a>
                </div>
        <?php
            foreach ($dev as $d):
            $fecha = new DateTime($d->post_date);
    ...
    function ver_archivo($year){
        global $wpdb;
        $inicio = sprintf("%s-01-01", $year);
        $fin = sprintf("%s-12-31", $year);
        $sql = sprintf("SELECT a.post_date FROM wp_posts AS a INNER JOIN wp_term_relationships AS b ON a.ID = b.object_id WHERE b.term_taxonomy_id = '34' AND a.post_date > '%s' AND a.post_date <= '%s' GROUP BY MONTH(a.post_date)", $inicio, $fin);
        $dev = $wpdb->get_results($sql);
        $fecha = new DateTime($dev[0]->post_date);
    ...
    function view_archive($year){
        ...
        $sql = sprintf("SELECT a.post_date FROM wp_post AS a INNER JOIN wp_term_relationships AS b ON a.ID = b.object_id where b.term_taxonomy_id = '34' AND a.post_date > '%s' AND a.post_date >= '%s' GROUP BY MONTH(a.post_date)", $start, $end);
        $dev = $wpdb-> get_results($sql);
    ...
    function enviar_esquela($from, $to, $mail, $content){
        ...
        $headers = 'From: Funeraria J. García López <noreply@jgarcialopez.com.mx>' . "\r\n";
        wp_mail($mail, 'Mensaje de JGL' ,sprintf("%s", stripslashes($content)), $headers);
        echo "ok";
    }
?>